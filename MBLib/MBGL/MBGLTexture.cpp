...
namespace MB
{
    MBGLTexture::MBGLTexture(MBGLContext * _ctx)
    {
        ctx = _ctx;
    ...
    unsigned int MBGLTexture::GL_GetTextureId()
    {
        return textureId;
    }

    int MBGLTexture::SetDataRedChannel(unsigned char * data,int width,int height)
    {
#ifdef QT_EYER_PLAYER
        ctx->glBindTexture(GL_TEXTURE_2D, textureId);
    ...
    int MBGLTexture::SetDataRGBAChannel(unsigned char * data,int width,int height)
    {
#ifdef QT_EYER_PLAYER
        ctx->glBindTexture(GL_TEXTURE_2D, textureId);
        ctx->glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
