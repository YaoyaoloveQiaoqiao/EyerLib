...
namespace MB
{
    MBGLVAO::MBGLVAO(MBGLContext * _ctx)
    {
        ctx = _ctx;
    ...
    int MBGLVAO::SetEBO(unsigned int * EBOdata, int bufferSize)
    {
        if(VAOId == 0){
            return -1;
        }
    ...
    int MBGLVAO::AddVBO(float * VBOdata, int bufferSize, int layout, int size, unsigned int stride)
    {
#ifdef QT_EYER_PLAYER
        ctx->glBindVertexArray(VAOId);
